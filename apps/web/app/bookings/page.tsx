...
export default function BookingsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-10">
      <h1 className="font-display text-4xl font-semibold text-ink">Your reservations</h1>

      {isLoading ? (
        <div className="mt-6 space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="card h-28 animate-pulse bg-surface2" />)}</div>
      ) : reservations.length ? (
            ...
            return (
              <Link key={r.id} href={`/bookings/${r.id}`} className="card p-5 block hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-display text-xl font-semibold text-ink">{r.vendor?.businessName || 'Restaurant'}</h3>
                    ...
                    <p className="mt-2 text-[12px] text-muted">Ref: <span className="font-mono font-medium text-ink">{r.reference}</span></p>
                  </div>
                ...
                {r.status === 'confirmed' && r.pin && (
                  <div className="mt-3 flex items-center gap-2 rounded-lg bg-[rgba(201,168,76,0.08)] px-3 py-2 text-[13px]">
                    ...
                    <span className="text-muted">Check-in PIN:</span>
                    <span className="font-bold tracking-widest text-ink">{r.pin}</span>
                  </div>
                )}
              </Link>
            );
        ...
        <div className="mt-12 text-center">
          <p className="text-lg text-ink">No reservations yet</p>
          <p className="text-sm text-muted mt-1">Find a table and your bookings will appear here.</p>
