  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-8">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl font-semibold text-[#0f2547]">Notifications</h1>
      ...
      {isLoading ? (
        <div className="mt-6 space-y-2">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="card h-16 animate-pulse bg-[#eef1f5]" />)}</div>
      ) : items.length ? (
            ...
            return (
              <button key={n.id} onClick={() => unread && markOne.mutate(n.id)} className={cn('w-full text-left flex gap-3 px-5 py-4', unread && 'bg-[rgba(201,168,76,0.05)]')}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-medium text-[#0f2547]">{n.title}</p>
                  {n.body && <p className="text-[13px] text-[#7a8fa6] mt-0.5">{n.body || n.message}</p>}
                  <p className="text-[11px] text-[#7a8fa6] mt-1">{formatDate(n.createdAt)}</p>
                </div>
        ...
        <div className="mt-12 text-center text-[#7a8fa6]">
          ...
          <p className="mt-3 text-lg text-[#0f2547]">No notifications</p>
