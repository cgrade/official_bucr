  ...
  return (
    <div className="max-w-xl mx-auto px-5 py-8">
      ...
      <div className="card p-6 mt-5">
        <p className="text-[12px] font-semibold uppercase tracking-wider text-[#7a8fa6]">Your referral code</p>
        <div className="mt-2 flex items-center justify-between rounded-xl bg-[rgba(201,168,76,0.1)] px-4 py-3">
          <span className="text-2xl font-bold tracking-widest text-[#0f2547]">{isLoading ? '…' : code}</span>
          <button onClick={() => copy(code)} className="text-[#c9a84c] hover:text-[#0f2547]"><Copy className="h-5 w-5" /></button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-5">
        <div className="card p-5 text-center">
          ...
          <p className="mt-2 text-2xl font-bold text-[#0f2547]">{stats.totalReferrals ?? stats.referralCount ?? 0}</p>
          <p className="text-[12px] text-[#7a8fa6]">Friends joined</p>
        </div>
        <div className="card p-5 text-center">
          <Gift className="h-5 w-5 mx-auto text-[#c9a84c]" />
          <p className="mt-2 text-2xl font-bold text-[#0f2547]">{stats.creditsEarned ?? stats.totalEarned ?? 0}</p>
          <p className="text-[12px] text-[#7a8fa6]">Credits earned</p>
        </div>
      </div>
    </div>
  );
