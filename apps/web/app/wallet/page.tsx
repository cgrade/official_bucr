...
export default function WalletPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-10">
      <h1 className="font-display text-4xl font-semibold text-[#0f2547]">Wallet</h1>
      ...
      <p className="text-[13px] text-[#7a8fa6] mb-4">1 credit = ₦10 value · ₦{PURCHASE_PRICE.toFixed(2)} to buy (6% spread). Valid for 90 days.</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {PACKAGES.map((credits) => (
          <button key={credits} onClick={() => setSelected(credits)}
            ...
            <p className="text-2xl font-bold text-[#0f2547]">{credits}</p>
            <p className="text-[12px] text-[#7a8fa6]">credits</p>
            <p className="mt-2 text-[14px] font-semibold text-[#c9a84c]">{formatNaira(Math.ceil(credits * PURCHASE_PRICE))}</p>
            <p className="text-[11px] text-[#7a8fa6]">{formatNaira(credits * 10)} value</p>
          </button>
        ))}
      </div>
      ...
      <p className="mt-3 text-center text-[12px] text-[#7a8fa6]">Secure payment via Paystack · card, transfer, USSD</p>
    </div>
  );
}
