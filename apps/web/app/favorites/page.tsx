...
export default function FavoritesPage() {
  ...
  if (!ready || !isAuthenticated) return <div className="max-w-6xl mx-auto px-5 py-20 text-center text-[#7a8fa6]">Loading…</div>;

  return (
    <div className="max-w-6xl mx-auto px-5 py-10">
      <h1 className="font-display text-4xl font-semibold text-[#0f2547] flex items-center gap-2"><Heart className="h-7 w-7 text-[#c9a84c]" fill="#c9a84c" /> Saved</h1>
      {isLoading ? (
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="card aspect-[4/3] animate-pulse bg-[#eef1f5]" />)}</div>
      ) : favorites.length ? (
        ...
        <div className="mt-12 text-center">
          ...
          <p className="text-lg text-[#0f2547] mt-3">No saved restaurants yet</p>
          <p className="text-sm text-[#7a8fa6]">Tap the heart on any restaurant to save it here.</p>
