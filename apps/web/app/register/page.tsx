  ...
  return (
    <div className="max-w-md mx-auto px-5 py-14">
      <div className="text-center mb-8">
        ...
        <h1 className="mt-6 font-display text-3xl font-semibold text-ink">Create your account</h1>
        <p className="mt-1 text-[14px] text-muted">Book tables that are actually waiting for you.</p>
      </div>
      <form onSubmit={onSubmit} className="card p-6 space-y-4">
        <div>
          <label className="block text-[13px] font-medium text-ink mb-1.5">Country</label>
            ...
            {COUNTRIES.map((c) => (
              <button key={c.name} type="button" onClick={() => setCountry(c.name)}
                className={cn('h-10 rounded-lg border text-[13px] font-medium transition-colors',
                  country === c.name ? 'border-[#c9a84c] bg-[rgba(201,168,76,0.12)] text-ink' : 'border-line text-muted hover:border-[#c9a84c]')}>
                {c.name}
              </button>
            ))}
          </div>
          <p className="mt-1.5 text-[11px] text-muted">Sets your phone format · prices shown in {cfg.currency}</p>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <span className="inline-flex items-center px-3 rounded-xl border border-line bg-[var(--fill)] text-[14px] font-medium text-[#c9a84c]">{cfg.dialCode}</span>
          ...
          {form.password.length > 0 && (
            <div className="mt-2 grid grid-cols-2 gap-1.5 text-[11px]">
              {[['8+ characters', pw.length], ['Uppercase', pw.upper], ['Lowercase', pw.lower], ['Number', pw.number]].map(([label, ok]) => (
                <span key={label as string} className={cn('flex items-center gap-1', ok ? 'text-emerald-600' : 'text-muted')}>
