...
export default function HomePage() {
  ...
  return (
    <>
        ...
        <section className="max-w-6xl mx-auto px-5 py-14">
          <div className="flex items-end justify-between mb-6">
            <div>
              ...
              <h2 className="font-display text-3xl font-semibold text-[#0f2547]">Spotlight restaurants</h2>
            </div>
      ...
      {/* How it works */}
      <section id="how-it-works" className="bg-[#f5f0e8]">
        <div className="max-w-6xl mx-auto px-5 py-16">
          <div className="text-center mb-10">
            <p className="text-[12px] font-semibold tracking-wider uppercase text-[#c9a84c]">How it works</p>
            <h2 className="font-display text-3xl md:text-4xl font-semibold text-[#0f2547]">Booking that actually holds</h2>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((s, i) => (
              <div key={s.title} className="card p-6">
                ...
                <h3 className="font-semibold text-[#0f2547] mb-1.5">{s.title}</h3>
      ...
      <section className="max-w-6xl mx-auto px-5 py-14">
        <div className="flex items-end justify-between mb-6">
          <h2 className="font-display text-3xl font-semibold text-[#0f2547]">Popular right now</h2>
          <Link href="/restaurants" className="text-[14px] font-medium text-[#c9a84c] hover:underline flex items-center gap-1">
            See all <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
        {isLoading ? (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {Array.from({ length: 4 }).map((_, i) => <div key={i} className="card aspect-[4/3] animate-pulse bg-[#eef1f5]" />)}
          </div>
        ) : vendors.length ? (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <p className="text-[#7a8fa6]">No restaurants yet — check back soon.</p>
        )}
      </section>
    </>
  );
