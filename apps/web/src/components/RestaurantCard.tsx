    ...
    <Link href={`/venue/${vendor.slug}`} className="group block card overflow-hidden hover:shadow-md transition-shadow">
      <div className="relative aspect-[4/3] bg-surface2 overflow-hidden">
        ...
        {vendor.bookWithConfidence && (
          <span className="absolute top-3 left-3 inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-semibold text-ink shadow-sm">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-[19px] font-semibold text-ink leading-tight flex items-center gap-1.5">
            {vendor.businessName}
          ...
          {vendor.averageRating ? (
            <span className="flex items-center gap-1 text-[13px] font-semibold text-ink flex-shrink-0">
