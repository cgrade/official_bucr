...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--bg-blur)] backdrop-blur border-b border-line">
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2">
          <BucrWordmark height={28} />
        </Link>

        <nav className="hidden md:flex items-center gap-7 text-[14px] font-medium text-ink">
        ...
        <div className="flex items-center gap-2">
          <Link href="/restaurants" className="md:hidden p-2 rounded-lg hover:bg-[var(--fill)]" aria-label="Search">
            <Search className="h-5 w-5 text-ink" />
          </Link>
          <ThemeToggle />
          {isAuthenticated ? (
            <>
              <Link href="/notifications" className="hidden sm:flex h-9 w-9 items-center justify-center rounded-lg hover:bg-[var(--fill)]" aria-label="Notifications">
                <Bell className="h-5 w-5 text-ink" />
              </Link>
              <Link href="/wallet" className="flex items-center gap-1.5 px-3 h-9 rounded-lg bg-[rgba(201,168,76,0.12)] text-ink text-[13px] font-semibold hover:bg-[rgba(201,168,76,0.2)] transition-colors">
              ...
              <button onClick={() => { logout(); router.push('/'); }} className="hidden sm:block text-[13px] text-muted hover:text-ink ml-1">
                Sign out
              </button>
            </>
          ) : (
            <>
              <Link href="/login" className="px-3 h-9 inline-flex items-center text-[14px] font-medium text-ink hover:text-[#c9a84c]">
                Sign in
              </Link>
