      ...
      {Array.from({ length: pageSize }).map((_, i) => (
        <div key={i} className="flex items-center space-x-4 p-4">
          {columns.map((_, j) => (
            <div
              key={j}
              className="h-4 bg-slate-200 bg-[rgba(255,255,255,0.04)] rounded animate-pulse flex-1"
            />
          ))}
        </div>
      ))}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {isLoading ? (
              ...
              data.map((item, index) => (
                <tr key={index} className="hover:bg-slate-50 hover:bg-[rgba(255,255,255,0.06)]/50 transition-colors">
                  {columns.map((column) => (
                    <td
                      key={String(column.key)}
                      className={cn(
