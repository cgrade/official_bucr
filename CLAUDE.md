...
|---|---|---|
| **Per-cover success fee** | Flat fee **per seated head**, billed to vendor **bi-weekly** (invoices issued on the 1st & 16th; a charge accrues on **every** check-in). The fee is multiplied by party size: **total = base × tier multiplier × partySize** (a table of 1 → 1×, a table of 4 → 4×). **Basic-tier base = N1,500 per head** (flat across all venue types). NEVER a percentage of the bill (a % would constitute settlement/licensing exposure). Reduced by subscription tier (Pro = 50% = N750/head, Elite = 0%). Triggered by guest-side check-in, not vendor self-report (prevents under-reporting). Vendors pay each invoice via Paystack (money flows **to** BUCR — closed-loop preserved) from the portal Billing page; **1-week grace**, **48h reminders** until paid, then overdue. Revenue recognised on payment, not issuance. | Stage 1 — launch. Sells with zero proof since it's pay-per-result. |
...
| Per-cover success fee | Built — segmented by venueType, reduced by subscription tier, accrues on **every** check-in, **invoiced bi-weekly** (1st & 16th) with a payable `CoverInvoice`, 1-week grace, 48h reminders, overdue flagging; vendor pays via Paystack on the portal **Billing** page (money flows to BUCR), revenue recognised on payment |
| Vendor message center | Built — `VendorMessage` inbox in the vendor portal + sidebar unread badge: receives new-reservation, cancellation, per-cover invoice + reminder, and admin broadcast entries (categorised); important categories also emailed. Admin sends broadcasts from the admin **Broadcast** page |
| Vendor subscriptions | Built — repriced to Basic free / Pro N30k / Elite N85k. **Scheduled downgrade** (`VendorSubscription.scheduledTier`): keep the current paid tier until period end (no refund — closed loop), cron applies it; paid landings prompt a re-subscribe since there's no saved card. `POST/DELETE /vendor/subscription/downgrade` + portal banner/controls. |
| Waitlist seating | Built — guests join a no-deposit "notify me" list; vendor portal **Waitlist** page can **notify** (15-min hold + push/SMS/email), **seat**, or remove the next party; guest `GET /waitlist` shows live position; cleanup cron + lazy-read expire stale holds (`ECONOMICS.WAITLIST_HOLD_MINUTES`). |
| Multi-branch booking | Built (core) — vendors CRUD branches (delete blocked while upcoming reservations exist; main protected); **diners pick a branch at booking** on web + mobile when >1 exists (`branchId` persisted). Per-branch hours UI + branch-filtered dashboards deferred (P2). |
| Booking verification gate | Built — a diner needs a verified **email OR phone** before booking (`ECONOMICS.REQUIRE_VERIFICATION_TO_BOOK`, default on). Register auto-sends the email OTP; `/reservations` returns `VERIFICATION_REQUIRED`; web + mobile have a verify screen; `/auth/me` exposes `isVerified`. |
...
| Vendor staff logins | Built — `VendorStaff` model with **per-staff granular permissions** + **seat limits by tier (Pro 1 / Elite 3, `ECONOMICS.VENDOR_STAFF_SEATS`)**. Owner invites (with role preset + custom permissions) → email accept link → set password → login at the vendor portal. Scoped to one vendor via `getVendorContext(payload)`; owner can delegate only within their own privileges (`VENDOR_DELEGATABLE_PERMISSIONS` — check_in, cancel_reservation, etc.); money/identity/team are never delegatable. Operational endpoints gated by `can(ctx, capability)`. Vendor portal **Team** page manages it. |
| Admin RBAC | Built — `super_admin` has full system access and is the only role that can create/edit/remove admins + set/remove roles (`/api/admin/admins`). Role presets (ops_manager, support, finance, content, analyst) + permission catalog in `src/lib/auth/admin-rbac.ts`; `adminCan(admin, 'resource.action')` guards enforce least privilege (wired on admins, settings, platform-revenue, credits, documents). Admin portal **Admins** page manages it. |
