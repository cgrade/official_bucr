...
/** GET /api/vendor/messages — the vendor's message center (owner + staff can read). */
export async function GET(request: NextRequest) {
  try {
    ...
    const [messages, unreadCount] = await Promise.all([
      db.vendorMessage.findMany({
        where: { vendorId: ctx.vendor.id },
        orderBy: { createdAt: 'desc' },
        take: 50,
      }),
      db.vendorMessage.count({ where: { vendorId: ctx.vendor.id, isRead: false } }),
    ]);

    return successResponse({ messages, unreadCount });
  } catch (error) {
